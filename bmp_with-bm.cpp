...
BMP_BM::BMP_BM(const std::string &filename) : BMP(filename) {
    ...
    if (infoHeader.biCompression == 3) {
        const auto invalidBitmask = [&]() -> void {
            std::cerr
                    << "BMP_BM: Invalid bitmasks. For 16-bit, the pixels will be treated as RGB565. For 32-bit, the pixels will be treated as RGB888"
                    << std::endl;
        };

        if (fileHeader.bfOffBits - fileHeaderSize - infoHeader.biSize != 32 * 3 / 8)
            invalidBitmask();
        else {
            // Read bitmask
